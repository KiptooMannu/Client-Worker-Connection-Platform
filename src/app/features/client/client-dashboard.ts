      ...
      <section class="mb-12 space-y-6">
        ...
        <div class="bg-white p-6 rounded-3xl shadow-[0_4px_24px_rgba(4,22,39,0.06)] border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-6">
          <div class="space-y-2">
            ...
            <div class="flex items-center border border-slate-200 rounded-xl px-4 py-3 bg-white focus-within:border-[#041627] focus-within:ring-2 focus-within:ring-[#041627]/10 transition-all">
      ...
      <div class="grid grid-cols-12 gap-8">
        ...
        <aside class="hidden lg:block lg:col-span-3 space-y-8">
          <div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
            <h3 class="text-xl font-black text-[#041627] mb-6">Market Insights</h3>
            <ul class="space-y-6">
              <li class="flex items-center justify-between">
                <span class="text-sm font-medium text-slate-500">Active Experts</span>
                <span class="text-sm font-black text-[#041627]">12,482</span>
              </li>
              <li class="flex items-center justify-between">
                <span class="text-sm font-medium text-slate-500">Avg. Hourly Rate</span>
                <span class="text-sm font-black text-[#041627]">$145/hr</span>
              </li>
              <li class="flex items-center justify-between">
              ...
              <div class="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-[0_4px_24px_rgba(4,22,39,0.04)] hover:shadow-[0_12px_48px_rgba(4,22,39,0.1)] transition-all group">
                <div class="flex items-start justify-between mb-6">
                  <div class="flex items-center gap-5">
                    ...
                    <div>
                      <h3 class="text-xl font-black text-[#041627] group-hover:text-blue-700 transition-colors">{{ worker.name }}</h3>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-slate-50">
                  <div>
                    ...
                    <p class="text-2xl font-black text-[#041627] tracking-tighter">\${{ worker.rate }}<span class="text-xs font-medium text-slate-400 ml-1">/hr</span></p>
                  </div>
                  <button [routerLink]="['/client/profile', worker.id]" 
                          class="bg-[#041627] text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-slate-900/10">
                    View Profile
                  </button>
                </div>
              </div>
...
export class ClientDashboardPage {
  ...
  filteredWorkers = computed(() => {
    ...
    if (this.locationQuery()) {
      ...
      // In this prototype, we'll search the bio for location keywords if not explicitly in schema
      list = list.filter(w => w.bio.toLowerCase().includes(loc));
    }

    // Experience Filter
    if (this.selectedExperience() && this.selectedExperience() !== 'null') {
       const exp = this.selectedExperience()!.toLowerCase();
       list = list.filter(w => w.category.toLowerCase().includes(exp) || w.bio.toLowerCase().includes(exp));
    }
